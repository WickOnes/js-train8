...
function getUsersFromDatabase(array) {
  ...
  return Array.from(array, ({ id, name: firstName, age: years }) => ({
    id,
...
function findElementIndexes(arr, element) {
  ...
  if (Array.isArray(arr)) {
    let index = [];
    index.push(arr.indexOf(element), arr.lastIndexOf(element));
    return index;
  } else {
    [];
  }
...
function copyAndSwapElements(arr, target, start, end) {
  ...
  if (start < 0 || end > arr.length || target < 0 || target > arr.length) {
